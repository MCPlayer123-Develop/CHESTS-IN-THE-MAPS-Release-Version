...
void update_mobs_overworld(){
	...
	for (int i = 1; i <= skeleton_sum; i++) { //骷髅射箭
		if (skeleton[i].x == x) {
			if (skeleton[i].y < y && overworld[skeleton[i].x][skeleton[i].y + 1] < 5 && abs(skeleton[i].x - x) < 5 && abs(skeleton[i].y - y) < 5) {
				arrow_sum++;
			...
			} else if (skeleton[i].y > y && overworld[skeleton[i].x][skeleton[i].y - 1] < 5 && abs(skeleton[i].x - x) < 5 && abs(skeleton[i].y - y) < 5) {
				arrow_sum++;
				arrow[arrow_sum].x = skeleton[i].x, arrow[arrow_sum].y = skeleton[i].y - 1, arrow[arrow_sum].facing = 2;
				overworld[arrow[arrow_sum].x][arrow[arrow_sum].y] = 17;
			}
		} else if (skeleton[i].y == y) {
			if (skeleton[i].x < x && overworld[skeleton[i].x + 1][skeleton[i].y] < 5 && abs(skeleton[i].x - x) < 5 && abs(skeleton[i].y - y) < 5) {
				arrow_sum++;
				arrow[arrow_sum].x = skeleton[i].x + 1, arrow[arrow_sum].y = skeleton[i].y, arrow[arrow_sum].facing = 1;
				overworld[arrow[arrow_sum].x][arrow[arrow_sum].y] = 17;
			} else if (skeleton[i].x > x && overworld[skeleton[i].x - 1][skeleton[i].y] < 5 && abs(skeleton[i].x - x) < 5 && abs(skeleton[i].y - y) < 5) {
				arrow_sum++;
...
void update_mobs_nether(){
	for (int i = 1; i <= lava_zombie_sum; i++) { //熔岩僵尸移动
		if (abs(lava_zombie[i].x - x) < 5 && abs(lava_zombie[i].y - y) < 5 && lava_zombie[i].x != 0 && lava_zombie[i].y != 0) {
	...
	for (int i = 1; i <= burn_skeleton_sum; i++) { //燃烧骷髅移动
		if (abs(burn_skeleton[i].x - x) < 5 && abs(burn_skeleton[i].y - y) < 5 && burn_skeleton[i].x != 0 && burn_skeleton[i].y != 0) {
	...
	for (int i = 1; i <= burn_skeleton_sum; i++) { //燃烧骷髅射火矢
		if (burn_skeleton[i].x == x) {
			if (burn_skeleton[i].y < y && nether[burn_skeleton[i].x][burn_skeleton[i].y + 1] < 2 && abs(burn_skeleton[i].x - x) < 5 && abs(burn_skeleton[i].y - y) < 5) {
				burn_arrow_sum++;
			...
			} else if (burn_skeleton[i].y > y && nether[burn_skeleton[i].x][burn_skeleton[i].y - 1] < 2 && abs(burn_skeleton[i].x - x) < 5 && abs(burn_skeleton[i].y - y) < 5) {
				burn_arrow_sum++;
			...
			if (burn_skeleton[i].x < x && nether[burn_skeleton[i].x + 1][burn_skeleton[i].y] < 2 && abs(burn_skeleton[i].x - x) < 5 && abs(burn_skeleton[i].y - y) < 5) {
				burn_arrow_sum++;
				...
				nether[burn_arrow[burn_arrow_sum].x][burn_arrow[burn_arrow_sum].y] = 15;
			} else if (burn_skeleton[i].x > x && nether[burn_skeleton[i].x - 1][burn_skeleton[i].y] < 2 && abs(burn_skeleton[i].x - x) < 5 && abs(burn_skeleton[i].y - y) < 5) {
				burn_arrow_sum++;
	...
	for (int i = 1; i <= blaze_sum; i++) { //烈焰人射火焰弹
		if (blaze[i].x == x) {
			if (blaze[i].y < y && nether[blaze[i].x][blaze[i].y + 1] < 2 && abs(blaze[i].x - x) < 5 && abs(blaze[i].y - y) < 5) {
				fire_charge_sum++;
			...
			} else if (blaze[i].y > y && nether[blaze[i].x][blaze[i].y - 1] < 2 && abs(blaze[i].x - x) < 5 && abs(blaze[i].y - y) < 5) {
				fire_charge_sum++;
				fire_charge[fire_charge_sum].x = blaze[i].x, fire_charge[fire_charge_sum].y = blaze[i].y - 1, fire_charge[fire_charge_sum].facing = 2;
				nether[fire_charge[fire_charge_sum].x][fire_charge[fire_charge_sum].y] = FIRE_CHARGE;
			}
		} else if (blaze[i].y == y) {
			if (blaze[i].x < x && nether[blaze[i].x + 1][blaze[i].y] < 2 && abs(blaze[i].x - x) < 5 && abs(blaze[i].y - y) < 5) {
				fire_charge_sum++;
				fire_charge[fire_charge_sum].x = blaze[i].x + 1, fire_charge[fire_charge_sum].y = blaze[i].y, fire_charge[fire_charge_sum].facing = 1;
				nether[fire_charge[fire_charge_sum].x][fire_charge[fire_charge_sum].y] = FIRE_CHARGE;
			} else if (blaze[i].x > x && nether[blaze[i].x - 1][blaze[i].y] < 2 && abs(blaze[i].x - x) < 5 && abs(blaze[i].y - y) < 5) {
				fire_charge_sum++;
