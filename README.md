...
- 映射表：`num_to_char_mp`（数字转显示字符）、`block_color`（控制台方块颜色）
### 3. 核心函数解析
```cpp
// Entity
...
# CHESTS IN THE MAPS
## Project Overview
*Chests in the Maps* is an adventure game in the style of Minecraft, available in two versions: the **C++ Console Version (Release Version)** and the **Web Version (Canary Version)**. The core gameplay revolves around "random map exploration → chest collection → monster slaying → Nether adventure → achievement completion", featuring a complete game system combined with a lightweight gaming experience.
---
...
|Achievement System|8 game achievements with real-time notification upon unlocking, guiding the gameplay flow|
### 2. Core Data Structures
#### Global Variables
- Maps: `overworld[N][N]` (Overworld), `nether[N][N]` (Nether)
- Player: coordinates `x/y`, facing `fx`, health `hp`, hunger `full`, money `money`
- Inventory: `inventory[30]` stores all item quantities
- Monsters: `MobData` struct storing coordinates, health, facing, managed in typed arrays
#### Enums & Mappings
- Block Enums: Define numeric IDs for all overworld/nether blocks (dirt, wall, chest, etc.)
- Mapping Tables: `num_to_char_mp` (number to display character), `block_color` (console block colors)
...
```
### 4. Monster AI
- Tracking: Automatically moves toward player within 3 tiles range
- Attack: Melee monsters deal damage up close; ranged monsters (Skeletons / Blazes) fire arrows / fire charges
- State: Switches display style upon taking damage, drops items on death
### 5. Controls
|Key|Function|
|---|---|
...
|`E`|View inventory|
|`Q`|Eat food to restore hunger|
|`U`|Use item|
|`P`|View achievements|
|`/`|Enter cheat command (requires cheats enabled)|
|`1`|Toggle debug mode|
|`R`|Return to main screen|
>Tip: You can rebind keys in Options
### 6. Game Flow
1. Launch: Start a new world or load a save
2. Overworld Adventure: Move → open chests → kill monsters for resources
3. Unlock Portal: Use Flint and Steel on the end portal, unlock achievement
4. Nether Challenge: Fight stronger monsters like Lava Zombies and Blazes
5. End Goal: Kill Blazes to obtain End Key, unlock final content
6. Death Mechanic: Survival mode drops items; Hardcore mode ends the game
### 7. Build Instructions
- Compilers: Dev-C++, Code::Blocks, Visual Studio
- Platform: Windows (depends on `windows.h`, `conio.h` console libraries)
- Save Files: Auto-generated `chest-game-data.dat` (map/inventory/monster data), `skin.txt` (custom player skin)
---
## Introduction to CHESTS IN THE MAPS Canary Version
The web version requires no download or installation — play directly in your browser. Lightweight design adapts to multiple devices, with core gameplay centered around "map exploration + chest collection", featuring a new Nether level as advanced content.
### 1. URL
[https://chests-game.pages.dev/](https://chests-game.pages.dev/)
### 2. Core Features
|Feature|Description|
|---|---|
|🎮 Start Game|Enter core gameplay, begin map exploration|
|🏆 Achievement Progress|View achievement completion status, clarify exploration goals|
|📂 Load Save|Load previous game progress, no need to restart|
|⚙️ Settings|Adjust basic gameplay parameters|
### 3. Core Gameplay Hints
- Core: Map exploration + chest collection/unlocking, with multiple difficulty map scenes and hidden chest clues
- Nether Level: Advanced content offering higher-challenge exploration mechanics and chest rewards
- Lightweight: No installation dependencies, runs directly in browser, simple controls
> Note: For more gameplay details (specific controls, chest rewards, map types), experience the full content via the "Start Game" page.
---
## Version Comparison
|Aspect|C++ Console Version|Web Version|
|---|---|---|
|Platform|Windows (console library)|Any browser (cross-platform)|
|Feature Completeness|Full systems (saves, cheats, AI, etc.)|Lightweight core gameplay|
|Difficulty|Requires compilation, developer-oriented|Instant play, player-oriented|
|Highlights|Source code study & custom extension|No installation, cross-device play|
---
## Target Audience
