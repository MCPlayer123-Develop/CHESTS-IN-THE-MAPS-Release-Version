...
string Blue_slogan[30] {
	...
	"Noooooooooooo!I want to play MAPS IN THE CHESTS!",
	"Where is the door???",
	"There's a chest!",
	"The colorful world!",
	"Aaa,you were been tricked!",
	"You look like a fool!",
	"Woo,Spooky!",
	"You should find a map in the chest."
};
string biome[10] = {
	"chest:Overworld",
	"chest:Nether",
...
void mp_fool() { //输出地图
	...
	for (int i = 1; i <= n; i++) {
		printf("%2d ", i);
		for (int j = 1; j <= n; j++) {
			SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE), block_color_fool[fool[i][j]]);
			printf("[%c]", num_to_char_mp_fool[fool[i][j]]);
			SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE), 0x03);
		}
		printf("\n");
	}
...
void make_world() {
	...
	for(int i = 1; i <= zombie_sum; i++) zombie[i] = {0, 0, 0, 0};
	for(int i = 1; i <= skeleton_sum; i++) skeleton[i] = {0, 0, 0, 0};
	for(int i = 1; i <= arrow_sum; i++) arrow[i] = {0, 0, 0, 0};
	int world_type = rand() % 4;
	if(world_type == 0){
		...
		for(int i = 1; i <= n; i++){
			for(int j = 1; j <= n; j++){
				int k = (i - 1) * n + j;
				zombie[k].x = i, zombie[k].y = j;
				overworld[zombie[k].x][zombie[k].y] = 9;
				zombie[k].hp = 3;
			}
		}
		...
		for(int i = 1; i <= n; i++){
			for(int j = 1; j <= n; j++){
				int k = (i - 1) * n + j;
				skeleton[k].x = i, skeleton[k].y = j;
				overworld[skeleton[k].x][skeleton[k].y] = 9;
				skeleton[k].hp = 3;
			}
		}
	}
...
void chest_fool(){
	...
	printf("╔═══════════════════════╗ \n");
	printf("║ 地图：");
	for(int i = 1; i <= 16; i++) cout << char(rand() % 95 + 32);
	printf("║ ");
	printf("\n╚═══════════════════════╝ \n");
	printf("准备进入！\n");
	system("pause");
...
void game_fool(){
	...
	while(1){
		...
		if (todo == 'd' || todo == 's' || todo == 'a' || todo == 'w') {
			...
			if (fx == 2) goY--, fx = 2;
			if (fx == 3) goX--, fx = 3;
			if (in(goX, goY) && (abs(goX - x) < 2 && abs(goY - y) < 2) && fool[goX][goY] == 2) {
				chest_fool();
				return ;
			}
		} else if (todo == '1') {
...
void game_nether() {
	...
	while (1) {
		if (gamemode == 1) hp = 20, full = 20, memset(inventory, 1, sizeof(inventory));
		for (int i = 1; i <= lava_zombie_sum; i++) {
...
void game_overworld() {
	...
	printf("\n\n\n\n\n");
	printf("						按任意键开始游戏...\n\n\n\n\n\n\n\n\n\n\n\n");
	printf("26y03m(April Fool's Day Version)\n");
	system("pause");
	system("cls");
	printf("存档\n");
