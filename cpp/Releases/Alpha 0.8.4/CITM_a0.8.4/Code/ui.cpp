...
int title_screen() {
    ...
    cout << title_slogan[slogan_p] << endl;
...
void useItem() {
    ...
    if (useSelect == 5 && inventory[17] > 0) {
        system("start ../Soundtrack/Happy_Chests.mp3");
    ...
    if (useSelect == 6 && inventory[18] > 0) {
        system("start ../Soundtrack/Stressful.mp3");
        SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE), 0x0E);
    ...
    if (useSelect == 7 && inventory[19] > 0) {
        system("start ../Soundtrack/Lava_Zombie.wav");
        SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE), 0x0E);
...
void view_inventory() {
    ...
    while (1) {
        ...
        if (todo == 'q' && gamemode == 1) {
            ...
            take_item_cnt = max(0, take_item_cnt);
            take_item_cnt = min(64, take_item_cnt);
