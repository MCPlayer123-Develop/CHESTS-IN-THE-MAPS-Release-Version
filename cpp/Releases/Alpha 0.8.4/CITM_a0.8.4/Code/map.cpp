...
void print_map() {
    printf(lang["map.border_top"].c_str());
    printf(lang["map.money"].c_str());
    SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE), 0x0E);
    printf("%9d", money);
    SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE), 0x03);
    printf(lang["map.health"].c_str());
    ...
    printf("%7d", hunger);
    SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE), 0x03);
    printf(lang["map.border_bottom"].c_str());
    printf("\n");
    printf("  ");
    for (int i = 1; i <= n; i++) {
        printf("%3d", i);
...
void generation_world_nether() {
    ...
    for (int i = 1; i <= n; i++) {
        for (int j = 1; j <= n; j++) {
            if (i == nether_mid - 1 || i == nether_mid || i == nether_mid + 1 || j == nether_mid - 1 || j == nether_mid
                || j == nether_mid + 1) {
                nether[i][j] = 1;
            } else {
                nether[i][j] = 0;
