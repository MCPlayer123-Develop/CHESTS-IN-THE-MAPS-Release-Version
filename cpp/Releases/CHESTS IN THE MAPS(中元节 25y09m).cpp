...
using namespace std;
const int N=55;
int n,a[N][N],sum[20],x,y,fx,money=0,hp=20,full=20,step=0,door_X,door_Y;
...
char printChar(int x) { //数转字符
	...
	if(x==11){
		if(abs((door_X)-x)<3&&abs((door_Y)-y)<3){
...
void game() {
	...
	printf("\n\n\n\n\n");
	printf("						按任意键开始游戏...\n\n\n\n\n\n\n\n\n\n\n\n\n");
	printf("Zhongyuan Festival 25y09m(中元节版)\n");
	system("pause");
	system("cls");
	printf("游戏选项:\n");
	...
	for(int level=1;level<=3;level++) {
		...
		for(int i=1; i<=zombieS; i++) {//生成僵尸  
			...
			if(zombieX[i]==door_X) zombieX[i]--;
			if(zombieY[i]==door_Y) zombieY[i]--;
		...
		for(int i=1; i<=skeletonS; i++){//生成骷髅 
			...
			if(skeletonX[i]==door_X) skeletonX[i]--;
			if(skeletonY[i]==door_Y) skeletonY[i]--;
...
//构建地图 
		while(1) {
			...
			for(int i=1;i<=skeletonS;i++){//骷髅射箭 
				if(skeletonX[i]==x&&abs(skeletonX[i]-x)<3&&abs(skeletonY[i]-y)<3){
				...
				else if(skeletonY[i]==y&&abs(skeletonX[i]-x)<3&&abs(skeletonY[i]-y)<3){
			...
			if(abs((door_X)-x)<3&&abs((door_Y)-y)<3){
				hp-=2;
				printf("玩家 被 门(chest:door) 攻击了\n");
			}
			if(abs((door_X)-x)<2&&abs((door_Y)-y)<2){
...
//游戏
	...
	int last[10],last_level=0;
	x=1;
	last[1]=10,last[2]=0,last[3]=0,last[4]=0,last[5]=11;
	while(1){
		if(last_level==5){
			break;
		}
		last[x]=10;
		printf("The last level...Died:%d\n",last_level);
		for(int i=1;i<=5;i++){
			printf("[%c]",printChar(last[i]));
			SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE),0x0C);
		}
		int c=getch();
		if(c=='a') last[x]=0,x--;
