...
void read_game_data() {
	...
	if (fp != NULL) {
		...
		for (int i = 1; i <= 8; i++) {
			int temp;
			fscanf(fp, "%d,", &temp);
			if (temp == 1) achievements[i] = 1;
			else achievements[i] = 0;
		}
		fscanf(fp, "\n");
		int temp;
		fscanf(fp, "is active portal = %d\n", &temp);
		if (temp == 1) is_active_portal = 1;
		else is_active_portal = 0;
		fscanf(fp, "debug screen = %d\n", &temp);
		if (temp == 1) debug_screen = 1;
		else debug_screen = 0;
...
void write_game_data() {
	...
	if (fl != NULL) {
		...
		for (int i = 1; i <= 8; i++) {
			int temp;
			if (achievements[i] == 1) temp = 1;
			else temp = 0;
			fprintf(fl, "%d,", temp);
		}
		fprintf(fl, "\n");
		int temp;
		if (is_active_portal == 1) temp = 1;
		else temp = 0;
		fprintf(fl, "is active portal = %d\n", temp);
		if (debug_screen == 1) temp = 1;
		else temp = 0;
...
int title_screen(){
	...
	printf("\n\n\n\n\n");
	printf("						     [1]开始游戏 [2]选项...\n\n\n\n\n\n\n\n\n\n\n\n");
	printf("Alpha 0.8.4-Dev-snapshot260417(测试版)\n");
