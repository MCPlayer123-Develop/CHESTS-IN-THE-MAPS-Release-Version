...
void game_overworld() {
	...
	if (is_regeneration_world == 1) {
		...
		while (1) {
			...
			printf("按w和s上下选择,Enter进入世界,q退出\n");
			for (int i = 0; i < saves.size(); i++) {
				if (select_world == i) {SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE), 0x0A);}
				printf("%s\n", saves[i].c_str());
				SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE), 0x03);
			}
			char todo = getch();
			...
			else if (todo == 13){world_name = saves[select_world]; break;}
...
void read_game_data() {
	...
	if (fp_skin == NULL) read_skin_path = "Saves/skin_" + world_name + ".txt", fp_skin = fopen(read_skin_path.c_str(), "r");
	if (fp_skin != NULL) {
...
void write_game_data() {
	...
	if (fl_skin == NULL) write_skin_path = "Saves/skin_" + world_name + ".txt", fl_skin = fopen(write_skin_path.c_str(), "w");
	if (fl_skin != NULL) {
...
void eggs_mcplayer123() {
	...
	while (1) {
		if (eggs_x == n && eggs_y == n) {
			system("cls");
			printf("MCPlayer123:请问您对这次的箱子旅行感觉如何?([0]满意 [1]不满意)\n");
			char eggs_select = getch();
			printf("MCPlayer123:我们会对您这次旅行收取费用,一共是$%d。\n", money);
			system("pause");
			money = 0;
			memset (inventory, 0, sizeof(inventory));
			write_game_data();
			system("start https://www.bilibili.com/video/BV1GJ411x7h7/?spm_id_from=333.337.search-card.all.click&vd_source=a0cef846b1c0f9cac5d8fbaabe6326e0");
			return ;
		}
		system("cls");
		for (int i = 1; i <= n; i++) {
			for (int j = 1; j <= n; j++) {
				if (eggs[i][j] == 0) {SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE), skin_color); cout << '[' << skin_char << ']';}
				else if (eggs[i][j] == 1) {SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE), 0xE0); cout << "[C]";}
				else if (eggs[i][j] == 2) {SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE), 0x80); cout << "[D]";}
				else if (eggs[i][j] == 3) {SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE), 0xF0); cout << "[ ]";}
			}
			cout << endl;
		}
		SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE), 0x03);
