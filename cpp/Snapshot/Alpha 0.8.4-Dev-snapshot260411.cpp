...
int n, inventory[30], x, y, facing, money = 0, health = 20, hunger = 20, hunger_step_summary = 0, Biome_Fest;
int overworld[N][N];
int nether[N][N];
...
bool vis[N][N], achievements[105], flag = 0, is_active_portal = 0, debug_screen = 0, is_first_level = 1, nether_return = 0;
int itemMoney[30] = {1, 5, 100, 100, 200, 300, 1000, 5000, 100000, 500000, 300000, 0, 0, 10, 0, 100000, 200000, 0, 0, 0, 0};
char key_bind[30] = {
	'd', 's', 'a', 'w', 'r', 'o', 'k', 'u', 'e', 'q', '/', '1', 'p'
};
...
void read_game_data() {
	...
	if (fp != NULL) {
		...
		fscanf(fp, "health = %d\n", &health);
		fscanf(fp, "hunger = %d\n", &hunger);
		fscanf(fp, "hunger_step_summary = %d\n", &hunger_step_summary);
		fscanf(fp, "biome = %d\n", &Biome_Fest);
		...
		fscanf(fp, "achievements:");
		for (int i = 1; i <= 8; i++) {
			int teprint_map;
			fscanf(fp, "%d,", &teprint_map);
			if (teprint_map == 1) achievements[i] = 1;
			else achievements[i] = 0;
		}
		fscanf(fp, "\n");
		int teprint_map;
		fscanf(fp, "is act door:%d\n", &teprint_map);
		if (teprint_map == 1) is_active_portal = 1;
		else is_active_portal = 0;
		fscanf(fp, "is debug:%d\n", &teprint_map);
		if (teprint_map == 1) debug_screen = 1;
		else debug_screen = 0;
...
void write_game_data() {
	...
	if (fl != NULL) {
		...
		fprintf(fl, "health = %d\n", health);
		fprintf(fl, "hunger = %d\n", hunger);
		fprintf(fl, "hunger_step_summary = %d\n", hunger_step_summary);
		fprintf(fl, "biome = %d\n", Biome_Fest);
		...
		fprintf(fl, "achievements:");
		for (int i = 1; i <= 8; i++) {
			int teprint_map;
			if (achievements[i] == 1) teprint_map = 1;
			else teprint_map = 0;
			fprintf(fl, "%d,", teprint_map);
		}
		fprintf(fl, "\n");
		int teprint_map;
		if (is_active_portal == 1) teprint_map = 1;
		else teprint_map = 0;
		fprintf(fl, "is act door:%d\n", teprint_map);
		if (debug_screen == 1) teprint_map = 1;
		else teprint_map = 0;
...
void options(){
	...
	while(1){
		...
		if(options_select == '0'){
			...
			printf("We will add language options in Alpha 0.8.4 soon... By CHESTS IN THE MAPS 游戏开发团队");
			Sleep(1000);
			...
			while(1){
				...
				char key_bind_select = getch();
				if(key_bind_select == 'w'){
					selecting = max (0, selecting - 1);
				} else if(key_bind_select == 's'){
					selecting = min (12, selecting + 1);
				} else if(key_bind_select == 13){
					cout << "请键入按键...";
					key_bind[selecting] = getch();
				} else if(key_bind_select == 'q'){
					break;
				}
				system("cls");
			}
...
void print_map() { //输出地图
	...
	if (debug_screen == 1) {
		...
		printf("hunger_step_summary for hunger:%d\n", hunger_step_summary);
		...
		for (int i = 1; i <= zombie_sum; i++) {
			printf("Zombie Coordinate:(%d,%d) health:%d\n", zombie[i].x, zombie[i].y, zombie[i].health);
		}
		printf("Skeleton Total:%d\n", skeleton_sum);
		for (int i = 1; i <= skeleton_sum; i++) {
			printf("Skeleton Coordinate:(%d,%d) health:%d\n", skeleton[i].x, skeleton[i].y, skeleton[i].health);
		}
...
void print_map_Nether() { //输出地图
	...
	if (debug_screen == 1) {
		...
		for (int i = 1; i <= lava_zombie_sum; i++) {
			printf("Lava Zombie Coordinate:(%d,%d) health:%d\n", lava_zombie[i].x, lava_zombie[i].y, lava_zombie[i].health);
		}
		printf("Burn Skeleton Total:%d\n", burn_skeleton_sum);
		for (int i = 1; i <= burn_skeleton_sum; i++) {
			printf("Burn Skeleton Coordinate:(%d,%d) health:%d\n", burn_skeleton[i].x, burn_skeleton[i].y, burn_skeleton[i].health);
		}
...
void shop() { //商店
	...
	while (1) {
		...
		if (shopSelect == 0) {
			...
			scanf("%d%d", &buy, &buyS);
			if (buy == 1) {
				if (money >= itemMoney[8]*buyS) {
					inventory[8] += buyS, money -= itemMoney[8] * buyS;
				} else {
					printf("您的金钱不足！\n");
					Sleep(2000);
				}
			} else if (buy == 2) {
				if (money >= itemMoney[9]*buyS) {
					inventory[9] += buyS, money -= itemMoney[9] * buyS;
				} else {
					printf("您的金钱不足！\n");
					Sleep(2000);
				}
			} else if (buy == 3) {
				if (money >= itemMoney[10]*buyS) {
					inventory[10] += buyS, money -= itemMoney[10] * buyS;
				} else {
					printf("您的金钱不足！\n");
					Sleep(2000);
				}
			} else if (buy == 4) {
				if (money >= itemMoney[13]*buyS) {
					inventory[13] += buyS, money -= itemMoney[13] * buyS;
				} else {
					printf("您的金钱不足！\n");
					Sleep(2000);
				}
		...
		if (shopSelect == 1) {
			printf("金钱:%d\n", money);
			for (int i = 0; i < 8; i++) {
				money += inventory[i] * itemMoney[i], inventory[i] = 0;
			}
			for (int i = 15 ; i < 17; i++) {
				money += inventory[i] * itemMoney[i], inventory[i] = 0;
			}
	...
	if (order[p] == "kill") {
		...
		if (order[p] == "chest:zombie") {
			...
			if (order[p] == "all") {
				...
				int teprint_map = stoi(order[p]);
				overworld[zombie[teprint_map].x][zombie[teprint_map].y] = 0;
				zombie[teprint_map] = {0, 0, 0, 0};
				printf("杀死了僵尸 %d \n", teprint_map);
				Sleep(1000);
			}
			...
			if (order[p] == "all") {
				...
				int teprint_map = stoi(order[p]);
				overworld[skeleton[teprint_map].x][skeleton[teprint_map].y] = 0;
				skeleton[teprint_map] = {0, 0, 0, 0};
				printf("杀死了骷髅 %d \n", teprint_map);
				Sleep(1000);
			}
			...
			if (order[p] == "all") {
				...
				int teprint_map = stoi(order[p]);
				nether[lava_zombie[teprint_map].x][lava_zombie[teprint_map].y] = 0;
				lava_zombie[teprint_map] = {0, 0, 0, 0};
				printf("杀死了熔岩僵尸 %d \n", teprint_map);
				Sleep(1000);
			}
			...
			if (order[p] == "all") {
				...
				int teprint_map = stoi(order[p]);
				nether[burn_skeleton[teprint_map].x][burn_skeleton[teprint_map].y] = 0;
				burn_skeleton[teprint_map] = {0, 0, 0, 0};
				printf("杀死了燃烧骷髅 %d \n", teprint_map);
				Sleep(1000);
			}
			...
			if (order[p] == "all") {
				...
				int teprint_map = stoi(order[p]);
				nether[blaze[teprint_map].x][blaze[teprint_map].y] = 0;
				blaze[teprint_map] = {0, 0, 0, 0};
				printf("杀死了烈焰人 %d \n", teprint_map);
				Sleep(1000);
			}
		} else if (order[p] == "chest:player") {
			health = 0, hunger = 1, died_Information = "You were been killed.";
			printf("杀死了玩家\n");
			Sleep(1000);
		} else if (order[p] == "all") {
			for (int i = 1; i <= zombie_sum; i++) {
...
void game_nether() {
	system("cls");
	system("color 0C");
	if ((is_regeneration_world == 1) || (is_regeneration_world == 0 && is_first_level == 0)) {
		system("color 5F");
		printf("\n\n\n\n\n\n                                                  下载地形中...");
		Sleep(3000);
		system("cls");
		system("color 0C");
		generation_world_nether();
	}
	while (1) {
		if (gamemode == 1) health = 20, hunger = 20, memset(inventory, 1, sizeof(inventory));
		for (int i = 1; i <= lava_zombie_sum; i++) {
		...
		if (hunger_step_summary > 0 && hunger_step_summary % 10 == 0 && hunger > 0) { //更新饱食度
			hunger--;
		}
		if (hunger == 20) {
			health++;
		...
		nether[x][y] = 2;
		char todo = getch();
		if (todo == key_bind[0] || todo == key_bind[1] || todo == key_bind[2] || todo == key_bind[3]) {
			int goX = x, goY = y;
			if (todo == key_bind[0]) goY++, facing = 0;
			if (todo == key_bind[1]) goX++, facing = 1;
			if (todo == key_bind[2]) goY--, facing = 2;
			if (todo == key_bind[3]) goX--, facing = 3;
		...
		} else if (todo == key_bind[11]) {
			debug_screen = !debug_screen;
		} else if (todo == key_bind[6]) {
		...
		} else if (todo == key_bind[8]) {
			view_inventory();
		} else if (todo == key_bind[9]) {
			eatFood();
		} else if (todo == key_bind[4]) {
			nether_return = 1;
			return ;
		} else if (todo == key_bind[10] && allow_command == 1) {
			command();
			SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE), 0x0C);
		} else if (todo == key_bind[10] && allow_command == 0) {
			SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE), 0x0C);
			printf("你没有作弊的权限!\n");
			Sleep(1000);
			SetConsoleTextAttribute(GetStdHandle(STD_OUTPUT_HANDLE), 0x0C);
		} else if (todo == key_bind[7]) {
			system("cls");
			useItem_nether();
		} else if (todo == key_bind[12]) {
			seeachievements();
			system("color 0C");
		}
		system("cls");
...
//构建地图
		while (1) {
			...
			if (x == n - 1 && y == n - 1 && is_active_portal == 1) {
				overworld[x][y] = 0;
				arrow_sum = 0;
				for (int i = 1; i <= arrow_sum; i++) {
					arrow[i].x = 0, arrow[i].y = 0;
